...
p galaxies.permutation(2).map(&:sort).uniq.sum { |g1,g2| (g1[0] - g2[0]).abs + (g1[1] - g2[1]).abs + double_rows.count { |row| Range.new(*[g1[0],g2[0]].sort).include?(row) } + double_cols.count { |col| Range.new(*[g1[1],g2[1]].sort).include?(col) } }