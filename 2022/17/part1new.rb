ALL_INPUT = ARGF.readline.chomp.chars
...
loop do
  piece = ALL_PIECES[rock_count % 4]
  move = ALL_INPUT[move_count % ALL_INPUT.length]

  puts "move #{move} #{move_count % ALL_INPUT.length}"
  print_board(render(board, piece_x, piece_y, piece, "@"))
  5.times { puts }
